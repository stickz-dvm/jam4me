  ...
  return (
    <motion.div
      ...
      className={`relative overflow-hidden rounded-xl ${
        currentlyPlaying ? "border-2 border-accent" : "border border-border"
    ...
    >
        ...
        {albumArt ? (
          <img
            ...
            className={`h-full w-full object-cover transition-all duration-500 ${
              currentlyPlaying ? "now-playing-poster scale-105" : "music-poster"
            }`}
            onError={handleImageError}
          />
        ) : (
          <div className={`flex h-full w-full items-center justify-center bg-muted ${
            currentlyPlaying ? "now-playing-poster" : ""
          }`}>
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-muted-foreground">
          ...
          <div className="flex items-center gap-1">
            <span className="price-badge text-xs">₦{song.price.toLocaleString()}</span>
          </div>
        </div>
